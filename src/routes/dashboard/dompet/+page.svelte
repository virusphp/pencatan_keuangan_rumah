<script lang="ts">
    ...
    async function fetchBalances() {
        ...
        try {
            isLoading = true;
            // Fetch the balances (transactions API conveniently returns this)
            ...
            if (data && data.accountBalances) {
                ...
                accountBalances = data.accountBalances.filter((acc: any) => 
                    acc.role_access === auth.session?.role || acc.role_access === 'all'
                );
            }
        } catch (error) {
            console.error(error);
...
<div class="space-y-6">
    <div class="flex items-center gap-3 bg-white dark:bg-surface-dark p-4 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800">
        ...
        <div>
            ...
            <p class="text-xs text-gray-500">Rincian saldo tiap-tiap akun Anda.</p>
        </div>
    </div>
    ...
    <div class="space-y-3">
        ...
        {:else if accountBalances.length === 0}
            <div class="text-center py-10 bg-white dark:bg-surface-dark rounded-3xl border border-dashed border-gray-300 dark:border-gray-700">
                <p class="text-gray-500 dark:text-gray-400">Belum ada dompet.</p>
            </div>
        {:else}
            {#each accountBalances as acc}
                <div class="bg-white dark:bg-surface-dark p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex items-center justify-between transition-transform hover:scale-[1.02]">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-full flex items-center justify-center bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                            </svg>
                        </div>
                    ...
                    <div class="text-right">
                        <p class="font-bold text-lg text-primary dark:text-blue-400">{formatRupiah(acc.balance)}</p>
                    </div>
                </div>
            {/each}
        {/if}
